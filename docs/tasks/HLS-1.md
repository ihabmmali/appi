...
- Record whether playback is Kodi-native, manually fixed, or InputStream Adaptive ABR and whether any rendition switch coincides with the stall.
...
## Evidence
Existing KNOWN_ISSUES entry preserved. DIAG-1 was linked on 2026-09-24 after the user requested an exportable debug mode specifically to collect evidence for this investigation. HLS-2 was linked after source review showed Appi's Kodi-default mode and explicit InputStream Adaptive path are different behaviors. No new experiment, implementation or device verification has been performed in this documentation task.

## Outcome and next action
Collect a reproducible failing case and comparable working case, preferably using DIAG-1 once implemented; preserve the known-good playback behavior during diagnosis and record the active HLS selection mode.
