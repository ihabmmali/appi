    ...
    try:
        with _connect() as connection:
            if not force and connection.execute(
                ...
                return False
            connection.execute(
                'INSERT OR REPLACE INTO queue(cache_key, queued_at, payload) VALUES(?,?,?)',
                (key, int(time.time()), encode_focus(payload)),
            )
        return True
    except sqlite3.Error as exc:
...
def process_one():
    ...
    with _connect() as connection:
        row = connection.execute(
            'SELECT cache_key, payload FROM queue ORDER BY queued_at LIMIT 1'
        ).fetchone()
    if not row:
        return False
    key, encoded = row
